...
- Tracking: `scripts/mlflow_utils/tracking.py` permite envolver el entrenamiento con MLflow y registrar trazas/artefactos.
- Serving: `main_api.py` (API FastAPI) y `main_streamlit.py` (app Streamlit) usan el mismo preprocesador/selección y modelo entrenado.
...
8. Entrenamiento con MLflow (tracking local):
   ...
   if "xgboost" in catalog: model_names.append("xgboost")
...
## Servir el modelo: API y Streamlit
- API FastAPI (`main_api.py`):
  - Arranque: `uvicorn main_api:app --host 0.0.0.0 --port 8000`
  - Artefactos: `models_fast/<modelo>.joblib`, `data/processed/preprocessor.joblib`, `data/selected/metadata_selection.json` (si hubo selección).
  - Endpoints: `/health`, `/` (info), `/predict` (registros crudos -> predicciones).
- App Streamlit (`main_streamlit.py`):
  - Arranque: `streamlit run main_streamlit.py`
  - Selecciona un modelo de `models_fast/`, ingresa valores manuales o sube CSV/Excel (requiere `openpyxl` para .xlsx).
  - Muestra un resumen amigable de la prediccion y permite descargar resultados en CSV.
  - Archivo de prueba: `sample_input.xlsx` en el root del repo.

## Notebooks y experiments
- `notebooks/`: EDA y showcase de modelado (ejecución rápida con MLflow).
- `experiments/`: salidas de EDA (correlaciones, histogramas, PCA, Prophet, etc.).
